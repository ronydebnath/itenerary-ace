...
export default function ManageCurrenciesPage() {
  ...
  return (
    <main className="min-h-screen bg-background p-4 md:p-8"><div className="container mx-auto py-8">
      ...
      <Card className="shadow-lg mt-8"><CardHeader><CardTitle className="text-xl flex items-center"><Tag className="mr-2 h-5 w-5"/>Manage Specific Currency Markups</CardTitle><CardDescription>Define markup percentages for specific currency pairs. These override the global markup. Last API fetch: <span className="font-semibold text-primary">{lastFetchedDate}</span></CardDescription></CardHeader><CardContent>{isLoading ? (<div className="flex justify-center items-center py-8"><Loader2 className="h-8 w-8 animate-spin text-primary" /><p className="ml-2">Loading specific markups...</p></div>) : (<div className="space-y-4"><div className="flex justify-end"><Button onClick={openNewSpecificMarkupDialog} size="sm" disabled={isLoadingCustomCurrencies || allManagedCurrencyCodes.length < 2}><PlusCircle className="mr-2 h-4 w-4" /> Add Specific Markup</Button></div>{specificMarkupRates.length === 0 ? (<p className="text-muted-foreground text-center py-4">No specific markups defined.</p>) : (<div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead onClick={() => requestSpecificMarkupSort('fromCurrency')} className="cursor-pointer hover:bg-muted/50"><div className="flex items-center">From {getSortIcon('fromCurrency', specificMarkupSortConfig)}</div></TableHead><TableHead onClick={() => requestSpecificMarkupSort('toCurrency')} className="cursor-pointer hover:bg-muted/50"><div className="flex items-center">To {getSortIcon('toCurrency', specificMarkupSortConfig)}</div></TableHead><TableHead onClick={() => requestSpecificMarkupSort('markupPercentage')} className="text-right cursor-pointer hover:bg-muted/50"><div className="flex items-center justify-end">Markup % {getSortIcon('markupPercentage', specificMarkupSortConfig)}</div></TableHead><TableHead className="text-center">Actions</TableHead></TableRow></TableHeader><TableBody>{sortedSpecificMarkups.map((sm) => (<TableRow key={sm.id}><TableCell>{sm.fromCurrency}</TableCell><TableCell>{sm.toCurrency}</TableCell><TableCell className="text-right font-mono">{sm.markupPercentage.toFixed(2)}%</TableCell><TableCell className="text-center"><Button variant="ghost" size="icon" onClick={() => openEditSpecificMarkupDialog(sm)} className="mr-2 text-primary hover:bg-primary/10 h-8 w-8"><Edit className="h-4 w-4" /></Button><Dialog><DialogTrigger asChild><Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10 h-8 w-8"><Trash2 className="h-4 w-4" /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Confirm Deletion</DialogTitle><AlertDescription>Delete specific markup from {sm.fromCurrency} to {sm.toCurrency}?</AlertDescription></DialogHeader><DialogFooter><DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose><Button variant="destructive" onClick={() => deleteSpecificMarkup(sm.id)}>Delete</Button></DialogFooter></DialogContent></Dialog></TableCell></TableRow>))}</TableBody></Table></div>)}</div>)}</CardContent></Card>

      <Card className="shadow-lg mt-8"><CardHeader className="flex flex-row justify-between items-center"><div><CardTitle className="text-xl">Manage Base Exchange Rates</CardTitle><CardDescription>Define base rates. "Effective Rate" includes logic &amp; markup ({globalMarkupPercentage}% global, or specific if set). Last API fetch: <span className="font-semibold text-primary">{lastFetchedDate}</span></CardDescription></div><div className="flex flex-col sm:flex-row gap-2"><Button onClick={() => refreshRates()} size="sm" variant="outline" className="border-primary text-primary hover:bg-primary/10"><RefreshCw className="mr-2 h-4 w-4" /> Refresh from API</Button><Button onClick={openNewRateDialog} size="sm" disabled={isLoadingCustomCurrencies || allManagedCurrencyCodes.length < 2}><PlusCircle className="mr-2 h-4 w-4" /> Add New Rate</Button></div></CardHeader><CardContent>{isLoadingExchangeRates ? (<div className="flex justify-center items-center py-8"><Loader2 className="h-8 w-8 animate-spin text-primary" /><p className="ml-2">Loading rates...</p></div>) : exchangeRateError ? (<Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertTitle>Error</AlertTitle><AlertDescription>{exchangeRateError}</AlertDescription></Alert>) : sortedExchangeRates.length === 0 ? (<p className="text-muted-foreground text-center py-4">No exchange rates defined. Click "Add New Rate" or "Refresh".</p>) : (<div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead onClick={() => requestRateTableSort('fromCurrency')} className="cursor-pointer hover:bg-muted/50"><div className="flex items-center">From {getSortIcon('fromCurrency', rateTableSortConfig)}</div></TableHead><TableHead onClick={() => requestRateTableSort('toCurrency')} className="cursor-pointer hover:bg-muted/50"><div className="flex items-center">To {getSortIcon('toCurrency', rateTableSortConfig)}</div></TableHead><TableHead onClick={() => requestRateTableSort('rate')} className="text-right cursor-pointer hover:bg-muted/50"><div className="flex items-center justify-end">Defined Base Rate {getSortIcon('rate', rateTableSortConfig)}</div></TableHead><TableHead onClick={() => requestRateTableSort('effectiveFinalRate')} className="text-right cursor-pointer hover:bg-muted/50"><div className="flex items-center justify-end">Effective Final Rate {getSortIcon('effectiveFinalRate', rateTableSortConfig)}</div></TableHead><TableHead onClick={() => requestRateTableSort('markupAdded')} className="text-right cursor-pointer hover:bg-muted/50"><div className="flex items-center justify-end">Markup Added {getSortIcon('markupAdded', rateTableSortConfig)}</div></TableHead><TableHead onClick={() => requestRateTableSort('source')} className="cursor-pointer hover:bg-muted/50 text-center"><div className="flex items-center justify-center">Source {getSortIcon('source', rateTableSortConfig)}</div></TableHead><TableHead className="text-center">Actions</TableHead></TableRow></TableHeader><TableBody>{sortedExchangeRates.map((rate) => { const rateDetails = getRate(rate.fromCurrency, rate.toCurrency); let effectiveFinalRateDisplay = "N/A"; let markupAddedDisplay = "N/A"; if (rateDetails) { effectiveFinalRateDisplay = rateDetails.finalRate.toFixed(6); markupAddedDisplay = (rateDetails.finalRate - rateDetails.baseRate).toFixed(6); if (Math.abs(parseFloat(markupAddedDisplay)) < 0.0000001) markupAddedDisplay = "0.000000"; } else { effectiveFinalRateDisplay = rate.rate.toFixed(6); markupAddedDisplay = "Calc Error"; } return (<TableRow key={rate.id}><TableCell>{rate.fromCurrency}</TableCell><TableCell>{rate.toCurrency}</TableCell><TableCell className="text-right font-mono">{rate.rate.toFixed(6)}</TableCell><TableCell className="text-right font-mono">{effectiveFinalRateDisplay}</TableCell><TableCell className="text-right font-mono">{markupAddedDisplay}</TableCell><TableCell className="text-center"><TooltipProvider><Tooltip><TooltipTrigger asChild><Badge variant={rate.source === 'api' ? "secondary" : "outline"} className={rate.source === 'api' ? "bg-blue-100 text-blue-700 border-blue-300" : "bg-green-50 text-green-700 border-green-300"}>{rate.source?.toUpperCase() || 'N/A'}</Badge></TooltipTrigger><TooltipContent><p>{rate.source === 'api' ? 'Fetched from ExchangeRate-API' : 'Manually entered/edited'}. Updated: {format(parseISO(rate.updatedAt), "dd MMM yy HH:mm")}</p></TooltipContent></Tooltip></TooltipProvider></TableCell><TableCell className="text-center"><Button variant="ghost" size="icon" onClick={() => openEditRateDialog(rate)} className="mr-2 text-primary hover:bg-primary/10 h-8 w-8"><Edit className="h-4 w-4" /></Button><Dialog><DialogTrigger asChild><Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10 h-8 w-8"><Trash2 className="h-4 w-4" /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Confirm Deletion</DialogTitle><AlertDescription>Delete rate from {rate.fromCurrency} to {rate.toCurrency}?</AlertDescription></DialogHeader><DialogFooter><DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose><Button variant="destructive" onClick={() => deleteRate(rate.id)}>Delete</Button></DialogFooter></DialogContent></Dialog></TableCell></TableRow>); })}</TableBody></Table></div>)}</CardContent></Card>

      <Dialog open={isRateFormOpen} onOpenChange={(isOpen) => { setIsRateFormOpen(isOpen); if (!isOpen) setEditingRate(null); }}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>{editingRate ? 'Edit' : 'Add'} Base Exchange Rate</DialogTitle><AlertDescription>This is a direct base rate. System may use USD intermediate for conversions. API-fetched USD rates may override manual USD rates.</AlertDescription></DialogHeader><form onSubmit={rateForm.handleSubmit(handleRateFormSubmit)} className="space-y-4 py-4"><div><Label htmlFor="rateFromCurrency">From Currency</Label><Controller control={rateForm.control} name="fromCurrency" render={({ field }) => (<Select onValueChange={field.onChange} value={field.value} disabled={isLoadingCustomCurrencies || allManagedCurrencyCodes.length === 0}><SelectTrigger id="rateFromCurrency" className="mt-1"><SelectValue placeholder={isLoadingCustomCurrencies ? "Loading..." : "Select"} /></SelectTrigger><SelectContent>{allManagedCurrencyCodes.map(c => <SelectItem key={`from-${c}`} value={c}>{c}</SelectItem>)}</SelectContent></Select>)} />{rateForm.formState.errors.fromCurrency && <p className="text-xs text-destructive mt-1">{rateForm.formState.errors.fromCurrency.message}</p>}</div><div><Label htmlFor="rateToCurrency">To Currency</Label><Controller control={rateForm.control} name="toCurrency" render={({ field }) => (<Select onValueChange={field.onChange} value={field.value} disabled={isLoadingCustomCurrencies || allManagedCurrencyCodes.length === 0}><SelectTrigger id="rateToCurrency" className="mt-1"><SelectValue placeholder={isLoadingCustomCurrencies ? "Loading..." : "Select"} /></SelectTrigger><SelectContent>{allManagedCurrencyCodes.map(c => <SelectItem key={`to-${c}`} value={c}>{c}</SelectItem>)}</SelectContent></Select>)} />{rateForm.formState.errors.toCurrency && <p className="text-xs text-destructive mt-1">{rateForm.formState.errors.toCurrency.message}</p>}</div><div><Label htmlFor="rateValue">Rate (1 Unit of "From" = X Units of "To")</Label><Input id="rateValue" type="number" step="0.000001" {...rateForm.register("rate")} className="mt-1" />{rateForm.formState.errors.rate && <p className="text-xs text-destructive mt-1">{rateForm.formState.errors.rate.message}</p>}</div><div className="flex justify-end space-x-2 pt-2"><DialogClose asChild><Button type="button" variant="outline">Cancel</Button></DialogClose><Button type="submit" className="bg-primary hover:bg-primary/90 text-primary-foreground" disabled={isLoadingCustomCurrencies || allManagedCurrencyCodes.length < 2}>{editingRate ? 'Update' : 'Add'} Rate</Button></div></form></DialogContent></Dialog>
