...
export default function ManageCurrenciesPage() {
  ...
  return (
    <main className="min-h-screen bg-background p-4 md:p-8"><div className="container mx-auto py-8">
      ...
      <Card className="shadow-lg mt-8">
        <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                ...
                <Button onClick={openNewRateDialog} size="sm" disabled={isLoadingCustomCurrenciesHook || allManagedCurrencyCodesForDisplay.length < 2} className="w-full sm:w-auto"><PlusCircle className="mr-2 h-4 w-4" /> Add New Rate</Button>
            </div>
        </CardHeader>
        <CardContent>
            {isTableDataLoading ? (
                <div className="flex justify-center items-center py-8"><Loader2 className="h-8 w-8 animate-spin text-primary" /><p className="ml-2">Loading rates and currency data...</p></div>
            ) : exchangeRateError ? (
                ...
                <p className="text-muted-foreground text-center py-4">No exchange rates defined for active currencies. Click "Add New Rate" or "Refresh from API".</p>
            ) : (
                        ...
                        <TableBody>
                            {sortedExchangeRates.map((rate) => {
                                ...
                                if (rateDetails) {
                                    ...
                                    effectiveFinalRateDisplay = rate.rate.toFixed(6); // Fallback if getRate fails for some reason for this pair
      ...
      <Dialog open={isRateFormOpen} onOpenChange={(isOpen) => { setIsRateFormOpen(isOpen); if (!isOpen) setEditingRate(null); }}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>{editingRate ? 'Edit' : 'Add'} Base Exchange Rate</DialogTitle><AlertDescription>This is a direct base rate. System may use {REFERENCE_CURRENCY} intermediate for conversions. API-fetched {REFERENCE_CURRENCY} rates may override manual {REFERENCE_CURRENCY} rates.</AlertDescription></DialogHeader><form onSubmit={rateForm.handleSubmit(handleRateFormSubmit)} className="space-y-4 py-4"><div><Label htmlFor="rateFromCurrency">From Currency</Label><Controller control={rateForm.control} name="fromCurrency" render={({ field }) => (<Select onValueChange={field.onChange} value={field.value} disabled={isLoadingCustomCurrenciesHook || allManagedCurrencyCodesForDisplay.length === 0}><SelectTrigger id="rateFromCurrency" className="mt-1"><SelectValue placeholder={isLoadingCustomCurrenciesHook ? "Loading..." : "Select"} /></SelectTrigger><SelectContent>{allManagedCurrencyCodesForDisplay.map(c => <SelectItem key={`from-${c}`} value={c}>{c}</SelectItem>)}</SelectContent></Select>)} />{rateForm.formState.errors.fromCurrency && <p className="text-xs text-destructive mt-1">{rateForm.formState.errors.fromCurrency.message}</p>}</div><div><Label htmlFor="rateToCurrency">To Currency</Label><Controller control={rateForm.control} name="toCurrency" render={({ field }) => (<Select onValueChange={field.onChange} value={field.value} disabled={isLoadingCustomCurrenciesHook || allManagedCurrencyCodesForDisplay.length === 0}><SelectTrigger id="rateToCurrency" className="mt-1"><SelectValue placeholder={isLoadingCustomCurrenciesHook ? "Loading..." : "Select"} /></SelectTrigger><SelectContent>{allManagedCurrencyCodesForDisplay.map(c => <SelectItem key={`to-${c}`} value={c}>{c}</SelectItem>)}</SelectContent></Select>)} />{rateForm.formState.errors.toCurrency && <p className="text-xs text-destructive mt-1">{rateForm.formState.errors.toCurrency.message}</p>}</div><div><Label htmlFor="rateValue">Rate (1 Unit of "From" = X Units of "To")</Label><Input id="rateValue" type="number" step="0.000001" {...rateForm.register("rate")} className="mt-1" />{rateForm.formState.errors.rate && <p className="text-xs text-destructive mt-1">{rateForm.formState.errors.rate.message}</p>}</div><div className="flex justify-end space-x-2 pt-2"><DialogClose asChild><Button type="button" variant="outline">Cancel</Button></DialogClose><Button type="submit" className="bg-primary hover:bg-primary/90 text-primary-foreground" disabled={isLoadingCustomCurrenciesHook || allManagedCurrencyCodesForDisplay.length < 2}>{editingRate ? 'Update' : 'Add'} Rate</Button></div></form></DialogContent></Dialog>
      <Dialog open={isSpecificMarkupFormOpen} onOpenChange={(isOpen) => { setIsSpecificMarkupFormOpen(isOpen); if(!isOpen) setEditingSpecificMarkup(null);}}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>{editingSpecificMarkup ? 'Edit' : 'Add'} Specific Currency Markup</DialogTitle><AlertDescription>Define a markup percentage for a specific currency pair. This overrides the global markup.</AlertDescription></DialogHeader><form onSubmit={specificMarkupFormHook.handleSubmit(handleSpecificMarkupFormSubmit)} className="space-y-4 py-4"><div><Label htmlFor="smFromCurrency">From Currency</Label><Controller control={specificMarkupFormHook.control} name="fromCurrency" render={({ field }) => (<Select onValueChange={field.onChange} value={field.value} disabled={isLoadingCustomCurrenciesHook || allManagedCurrencyCodesForDisplay.length === 0}><SelectTrigger id="smFromCurrency" className="mt-1"><SelectValue placeholder={isLoadingCustomCurrenciesHook ? "Loading..." : "Select"} /></SelectTrigger><SelectContent>{allManagedCurrencyCodesForDisplay.map(c => <SelectItem key={`smfrom-${c}`} value={c}>{c}</SelectItem>)}</SelectContent></Select>)} />{specificMarkupFormHook.formState.errors.fromCurrency && <p className="text-xs text-destructive mt-1">{specificMarkupFormHook.formState.errors.fromCurrency.message}</p>}</div><div><Label htmlFor="smToCurrency">To Currency</Label><Controller control={specificMarkupFormHook.control} name="toCurrency" render={({ field }) => (<Select onValueChange={field.onChange} value={field.value} disabled={isLoadingCustomCurrenciesHook || allManagedCurrencyCodesForDisplay.length === 0}><SelectTrigger id="smToCurrency" className="mt-1"><SelectValue placeholder={isLoadingCustomCurrenciesHook ? "Loading..." : "Select"} /></SelectTrigger><SelectContent>{allManagedCurrencyCodesForDisplay.map(c => <SelectItem key={`smto-${c}`} value={c}>{c}</SelectItem>)}</SelectContent></Select>)} />{specificMarkupFormHook.formState.errors.toCurrency && <p className="text-xs text-destructive mt-1">{specificMarkupFormHook.formState.errors.toCurrency.message}</p>}</div><div><Label htmlFor="smMarkupPercentage">Markup Percentage (%)</Label><Input id="smMarkupPercentage" type="number" step="0.01" {...specificMarkupFormHook.register("markupPercentage")} className="mt-1" placeholder="e.g., 1.5 for 1.5%"/>{specificMarkupFormHook.formState.errors.markupPercentage && <p className="text-xs text-destructive mt-1">{specificMarkupFormHook.formState.errors.markupPercentage.message}</p>}</div><div className="flex justify-end space-x-2 pt-2"><DialogClose asChild><Button type="button" variant="outline">Cancel</Button></DialogClose><Button type="submit" className="bg-primary hover:bg-primary/90 text-primary-foreground" disabled={isLoadingCustomCurrenciesHook || allManagedCurrencyCodesForDisplay.length < 2}>{editingSpecificMarkup ? 'Update' : 'Add'} Markup</Button></div></form></DialogContent></Dialog>
    </div></main>
  );
}
