...
export function DetailsSummaryTable({ summary, currency }: DetailsSummaryTableProps) {
  const renderItemRow = (item: DetailedSummaryItem, isPrintView: boolean = false) => (
    <React.Fragment key={`${item.id}-${isPrintView ? 'print' : 'screen'}`}>
      ...
      {item.occupancyDetails && item.occupancyDetails.map((occDetail, index) => <TableRow key={`${item.id}-occ-${index}-${isPrintView ? 'print' : 'screen'}`} className={`bg-muted/30 text-xs ${isPrintView ? 'print-hotel-occupancy-detail-item' : 'hotel-occupancy-detail-item'}`}>
          <TableCell></TableCell>
  ...
  return (
      ...
      <Table>
        ...
        <TableBody>
          {Object.entries(groupedItems).length > 0
            ? Object.entries(groupedItems).map(([type, items]) => <React.Fragment key={type}>
    ...
    <div className="print-only hidden">
       <Table>
        ...
        <TableBody>
          {Object.entries(groupedItems).length > 0
             ? Object.entries(groupedItems).map(([type, items]) => <React.Fragment key={`${type}-print`}>
                <TableRow className="bg-secondary/50 category-summary-header">
                  <TableCell colSpan={5} className="font-bold text-secondary-foreground">{type}</TableCell>
                </TableRow>
                {items.map(item => <React.Fragment key={`${item.id}-print-detail`}>
                  ...
                  {item.occupancyDetails && item.occupancyDetails.map((occDetail, index) => <TableRow key={`${item.id}-occ-${index}-print`} className={`bg-muted/30 text-xs print-hotel-occupancy-detail-item`}>
                      <TableCell></TableCell>
                      <TableCell className="pl-6">└ Room: {occDetail.roomCategory}</TableCell>
                      <TableCell>#R: {occDetail.numRooms}, Nights: {occDetail.nights}</TableCell>
                      <TableCell className="font-code whitespace-pre-wrap max-w-xs">
